...
func (i *Image) Tree(ctx context.Context) (*bytes.Buffer, error) {
	...
	buildOutputBuffer(&buffer, rootImage, 0, "└─ ", treeDepth)

	return &buffer, err
}

func buildOutputBuffer(buffer *bytes.Buffer, root *Image, rootSize uint64, prefix string, treeDepth int) {
	if treeDepth == 0 {
		return
	}
	size, _ := root.Size(context.Background())
	//Size of current layer will be current layer size minus upper layer
	vSize := units.HumanSizeWithPrecision(float64(*size-rootSize), 4)
	buffer.WriteString(fmt.Sprintf("%s ID: %s  Tags: %v VirtualSize: %s\n", prefix, root.ID()[:12], root.Names(), vSize))
	...
	for _, child := range children {
		...
		nextPrefix := "  "
		buildOutputBuffer(buffer, img, *size, nextPrefix+prefix, treeDepth-1)
